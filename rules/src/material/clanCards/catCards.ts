 ...
 *
 * The half turn itself is written down as an effect of each face (see {@link halfTurn}), which is what makes that
 * round possible: a face with nothing else on it still gives something, and its square is activated like any
 * other. It also leaves the Rings out by simply not giving it, rather than by an exception made for them.
 */
...
/**
 * What each face of every other Cat card gives on top of what it prints: the half turn onto the other one
 * (see {@link Effect.HalfTurn}). A face that gives nothing else gives this alone, and that is enough.
 */
const halfTurn = { [Effect.HalfTurn]: 1 }
...
export const catCards = {
  ...
  [ClanCardId.CatSpendRingForToken]: {
    cost: { food: 5 },
    effects: { [Effect.SpendRingForToken]: 1, ...halfTurn },
    secondEffects: halfTurn
  },
  ...
  [ClanCardId.CatFoodAndMilitary]: {
    cost: { cards: 1 },
    effects: { [Effect.Food]: 1, [Effect.Military]: 1, ...halfTurn },
    secondEffects: halfTurn
  },
  ...
  [ClanCardId.CatDrawAndFood]: {
    cost: { food: 5 },
    effects: { [Effect.Draw]: 1, [Effect.Food]: 1, ...halfTurn },
    secondEffects: halfTurn
  },
  ...
  [ClanCardId.CatMilitaryOrUpgrade]: {
    cost: { food: 6 },
    effects: { [Effect.Military]: 2, ...halfTurn },
    secondEffects: { [Effect.Upgrade]: 1, ...halfTurn }
  },
